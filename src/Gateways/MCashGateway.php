...
class MCashGateway implements PaymentGatewayInterface
{
    ...
    public function __construct(array $config)
    {
        ...
        $this->baseUrl = $config['sandbox'] ? 
            'https://sandbox.mcash.com.bd/api/v1' : 
            'https://api.mcash.com.bd/api/v1';
    ...
    private function validateConfiguration(): void
    {
        ...
        foreach ($required as $key) {
            if (empty($this->config[$key])) {
                throw new ConfigurationException("MCash configuration missing: {$key}");
            }
        }
    }
    ...
    public function initializePayment(array $data): array
    {
        ...
        try {
            ...
            throw new PaymentException($responseData['message'] ?? 'Failed to initialize MCash payment');
        } catch (GuzzleException $e) {
            Log::error('MCash payment initialization failed', [
                'error' => $e->getMessage(),
                'data' => $data,
            ]);
            throw new NetworkException('MCash payment initialization failed: ' . $e->getMessage());
        }
    }
    ...
    public function verifyPayment(string $transactionId, array $data = []): array
    {
        try {
            ...
            if ($response->getStatusCode() === 200) {
                $status = $this->mapMCashStatus($responseData['data']['status'] ?? 'PENDING');
            ...
            throw new PaymentException('Failed to verify MCash payment');
        } catch (GuzzleException $e) {
            Log::error('MCash payment verification failed', [
                'error' => $e->getMessage(),
                'transaction_id' => $transactionId,
            ]);
            throw new NetworkException('MCash payment verification failed: ' . $e->getMessage());
        }
    }
    ...
    public function refundPayment(string $transactionId, ?float $amount = null, string $reason = ''): array
    {
        try {
            ...
            throw new PaymentException($responseData['message'] ?? 'Failed to refund MCash payment');
        } catch (GuzzleException $e) {
            Log::error('MCash payment refund failed', [
                'error' => $e->getMessage(),
                'transaction_id' => $transactionId,
            ]);
            throw new NetworkException('MCash payment refund failed: ' . $e->getMessage());
        }
    }
    ...
    private function mapMCashStatus(string $status): string
    {
        return match ($status) {
            'PENDING' => 'pending',
            'PROCESSING' => 'pending',
