...
class MyCashGateway implements PaymentGatewayInterface
{
    ...
    public function __construct(array $config)
    {
        ...
        $this->baseUrl = $config['sandbox'] ? 
            'https://sandbox.mycash.com.bd/api/v1' : 
            'https://api.mycash.com.bd/api/v1';
    ...
    private function validateConfiguration(): void
    {
        ...
        foreach ($required as $key) {
            if (empty($this->config[$key])) {
                throw new ConfigurationException("MyCash configuration missing: {$key}");
            }
        }
    }
    ...
    public function initializePayment(array $data): array
    {
        ...
        try {
            ...
            throw new PaymentException($responseData['message'] ?? 'Failed to initialize MyCash payment');
        } catch (GuzzleException $e) {
            Log::error('MyCash payment initialization failed', [
                'error' => $e->getMessage(),
                'data' => $data,
            ]);
            throw new NetworkException('MyCash payment initialization failed: ' . $e->getMessage());
        }
    }
    ...
    public function verifyPayment(string $transactionId, array $data = []): array
    {
        try {
            ...
            if ($response->getStatusCode() === 200) {
                $status = $this->mapMyCashStatus($responseData['data']['status'] ?? 'PENDING');
            ...
            throw new PaymentException('Failed to verify MyCash payment');
        } catch (GuzzleException $e) {
            Log::error('MyCash payment verification failed', [
                'error' => $e->getMessage(),
                'transaction_id' => $transactionId,
            ]);
            throw new NetworkException('MyCash payment verification failed: ' . $e->getMessage());
        }
    }
    ...
    public function refundPayment(string $transactionId, ?float $amount = null, string $reason = ''): array
    {
        try {
            ...
            throw new PaymentException($responseData['message'] ?? 'Failed to refund MyCash payment');
        } catch (GuzzleException $e) {
            Log::error('MyCash payment refund failed', [
                'error' => $e->getMessage(),
                'transaction_id' => $transactionId,
            ]);
            throw new NetworkException('MyCash payment refund failed: ' . $e->getMessage());
        }
    }
    ...
    private function mapMyCashStatus(string $status): string
    {
        return match ($status) {
            'PENDING' => 'pending',
            'PROCESSING' => 'pending',
