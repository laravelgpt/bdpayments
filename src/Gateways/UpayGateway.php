...
class UpayGateway implements PaymentGatewayInterface
{
    ...
    public function __construct(array $config)
    {
        ...
        $this->baseUrl = $config['sandbox'] ? 
            'https://sandbox.upay.com.bd/api/v1' : 
            'https://api.upay.com.bd/api/v1';
    ...
    private function validateConfiguration(): void
    {
        ...
        foreach ($required as $key) {
            if (empty($this->config[$key])) {
                throw new ConfigurationException("Upay configuration missing: {$key}");
            }
        }
    }
    ...
    public function initializePayment(array $data): array
    {
        ...
        try {
            ...
            throw new PaymentException($responseData['message'] ?? 'Failed to initialize Upay payment');
        } catch (GuzzleException $e) {
            Log::error('Upay payment initialization failed', [
                'error' => $e->getMessage(),
                'data' => $data,
            ]);
            throw new NetworkException('Upay payment initialization failed: ' . $e->getMessage());
        }
    }
    ...
    public function verifyPayment(string $transactionId, array $data = []): array
    {
        try {
            ...
            if ($response->getStatusCode() === 200) {
                $status = $this->mapUpayStatus($responseData['data']['status'] ?? 'PENDING');
            ...
            throw new PaymentException('Failed to verify Upay payment');
        } catch (GuzzleException $e) {
            Log::error('Upay payment verification failed', [
                'error' => $e->getMessage(),
                'transaction_id' => $transactionId,
            ]);
            throw new NetworkException('Upay payment verification failed: ' . $e->getMessage());
        }
    }
    ...
    public function refundPayment(string $transactionId, ?float $amount = null, string $reason = ''): array
    {
        try {
            ...
            throw new PaymentException($responseData['message'] ?? 'Failed to refund Upay payment');
        } catch (GuzzleException $e) {
            Log::error('Upay payment refund failed', [
                'error' => $e->getMessage(),
                'transaction_id' => $transactionId,
            ]);
            throw new NetworkException('Upay payment refund failed: ' . $e->getMessage());
        }
    }
    ...
    private function mapUpayStatus(string $status): string
    {
        return match ($status) {
            'PENDING' => 'pending',
            'PROCESSING' => 'pending',
