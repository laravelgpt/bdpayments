...
class AamarPayGateway implements PaymentGatewayInterface
{
    ...
    public function __construct(array $config)
    {
        ...
        $this->baseUrl = $config['sandbox'] ? 
            'https://sandbox.aamarpay.com/api/v1' : 
            'https://api.aamarpay.com/api/v1';
    ...
    {
        $required = ['store_id', 'signature_key', 'api_key'];
        
        foreach ($required as $key) {
            if (empty($this->config[$key])) {
                throw new ConfigurationException("AamarPay configuration missing: {$key}");
            }
        }
    }
    ...
    public function initializePayment(array $data): array
    {
        ...
        try {
            ...
            throw new PaymentException($responseData['message'] ?? 'Failed to initialize AamarPay payment');
        } catch (GuzzleException $e) {
            Log::error('AamarPay payment initialization failed', [
                'error' => $e->getMessage(),
                'data' => $data,
            ]);
            throw new NetworkException('AamarPay payment initialization failed: ' . $e->getMessage());
        }
    }
    ...
    public function verifyPayment(string $transactionId, array $data = []): array
    {
        try {
            ...
            if ($response->getStatusCode() === 200) {
                $status = $this->mapAamarPayStatus($responseData['data']['status'] ?? 'PENDING');
            ...
            throw new PaymentException('Failed to verify AamarPay payment');
        } catch (GuzzleException $e) {
            Log::error('AamarPay payment verification failed', [
                'error' => $e->getMessage(),
                'transaction_id' => $transactionId,
            ]);
            throw new NetworkException('AamarPay payment verification failed: ' . $e->getMessage());
        }
    }
    ...
    public function refundPayment(string $transactionId, ?float $amount = null, string $reason = ''): array
    {
        try {
            ...
            throw new PaymentException($responseData['message'] ?? 'Failed to refund AamarPay payment');
        } catch (GuzzleException $e) {
            Log::error('AamarPay payment refund failed', [
                'error' => $e->getMessage(),
                'transaction_id' => $transactionId,
            ]);
            throw new NetworkException('AamarPay payment refund failed: ' . $e->getMessage());
        }
    }
    ...
    private function mapAamarPayStatus(string $status): string
    {
        return match ($status) {
            'PENDING' => 'pending',
            'PROCESSING' => 'pending',
