...
class UCashGateway implements PaymentGatewayInterface
{
    ...
    public function __construct(array $config)
    {
        ...
        $this->baseUrl = $config['sandbox'] ? 
            'https://sandbox.ucash.com.bd/api/v1' : 
            'https://api.ucash.com.bd/api/v1';
    ...
    private function validateConfiguration(): void
    {
        ...
        foreach ($required as $key) {
            if (empty($this->config[$key])) {
                throw new ConfigurationException("UCash configuration missing: {$key}");
            }
        }
    }
    ...
    public function initializePayment(array $data): array
    {
        ...
        try {
            ...
            throw new PaymentException($responseData['message'] ?? 'Failed to initialize UCash payment');
        } catch (GuzzleException $e) {
            Log::error('UCash payment initialization failed', [
                'error' => $e->getMessage(),
                'data' => $data,
            ]);
            throw new NetworkException('UCash payment initialization failed: ' . $e->getMessage());
        }
    }
    ...
    public function verifyPayment(string $transactionId, array $data = []): array
    {
        try {
            ...
            if ($response->getStatusCode() === 200) {
                $status = $this->mapUCashStatus($responseData['data']['status'] ?? 'PENDING');
            ...
            throw new PaymentException('Failed to verify UCash payment');
        } catch (GuzzleException $e) {
            Log::error('UCash payment verification failed', [
                'error' => $e->getMessage(),
                'transaction_id' => $transactionId,
            ]);
            throw new NetworkException('UCash payment verification failed: ' . $e->getMessage());
        }
    }
    ...
    public function refundPayment(string $transactionId, ?float $amount = null, string $reason = ''): array
    {
        try {
            ...
            throw new PaymentException($responseData['message'] ?? 'Failed to refund UCash payment');
        } catch (GuzzleException $e) {
            Log::error('UCash payment refund failed', [
                'error' => $e->getMessage(),
                'transaction_id' => $transactionId,
            ]);
            throw new NetworkException('UCash payment refund failed: ' . $e->getMessage());
        }
    }
    ...
    private function mapUCashStatus(string $status): string
    {
        return match ($status) {
            'PENDING' => 'pending',
            'PROCESSING' => 'pending',
